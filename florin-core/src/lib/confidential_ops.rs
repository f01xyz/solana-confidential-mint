...
/// Perform a confidential transfer using an imported proof
/// This is the main function to use with proofs generated by florin-zk
pub async fn confidential_transfer(
    ...
    let proof = crate::proof_import::import_proof_from_file(proof_path)?;
    crate::proof_import::validate_proof(&proof)?;
    
    // Extract source and destination addresses from the proof
    let source_address = proof.metadata.source_address
        .as_ref()
        .ok_or_else(|| anyhow!("Missing source address in proof"))?;
    let source_account = Pubkey::try_from(source_address.as_str())
        .map_err(|_| anyhow!("Invalid source address format"))?;
        
    let destination_address = proof.metadata.destination_address
        .as_ref()
        .ok_or_else(|| anyhow!("Missing destination address in proof"))?;
    let destination_account = Pubkey::try_from(destination_address.as_str())
        .map_err(|_| anyhow!("Invalid destination address format"))?;
        
    // Extract amount (optional for some proof types)
    let amount = proof.metadata.amount
        .unwrap_or(0); // Default to 0 if not present
        
    // Get the mint address if available
    let mint_address = if let Some(mint_str) = proof.metadata.mint_address.as_ref() {
        Some(Pubkey::try_from(mint_str.as_str())
            .map_err(|_| anyhow!("Invalid mint address format"))?)
    } else {
        // If mint address not available in the proof, try to get it from the token account
        // This is a fallback for backward compatibility
        Some(get_token_mint(client.clone(), &source_account).await?)
    };
    
    // Verify the proof
    let config = crate::proof_verification::VerificationConfig {
        check_version: true,
        // Add other verification parameters as needed
    };
    ...
    if !verification_result.is_valid {
        return Err(anyhow!("Invalid proof: {}", verification_result.message.unwrap_or_default()));
    }
    
    // Process the proof based on its type
    match proof.proof_type {
        crate::proof_import::ProofType::Transfer => {
            // Use the transfer proof data
            perform_transfer(
                client,
                &source_account,
                &destination_account,
        ...
        crate::proof_import::ProofType::TransferWithProof => {
            // Legacy format
        ...
        crate::proof_import::ProofType::Withdraw => {
            // Use the withdraw proof data
            perform_withdraw(
                client,
                &source_account,
                owner,
        ...
        crate::proof_import::ProofType::WithdrawWithProof => {
            // Legacy format
...
/// Internal helper to perform a transfer with an imported proof
async fn perform_transfer(
    ...
    owner: &Keypair,
    amount: u64,
    proof: &crate::proof_import::ImportableProof,
) -> Result<String> {
    // Extract binary proof data
    let proof_data = crate::proof_import::extract_proof_binary_data(proof)?;
    ...
    // Get the mint from the token account
    let mint = get_token_mint(client.clone(), source_token_account).await?;
    ...
    // Call the confidential transfer instruction
    let signature = token
        .confidential_transfer_transfer_with_fee_with_proof(
            source_token_account,               // Source token account
            destination_token_account,          // Destination token account
            &owner.pubkey(),                    // Owner of source account
            amount,                             // Amount to transfer (for logging only)
            proof_data.as_slice(),              // The proof data 
            &[owner],                           // Required signers
        )
        .await?;
...
/// Internal helper to perform a withdraw with an imported proof
async fn perform_withdraw(
    client: Arc<RpcClient>,
    token_account: &Pubkey,
    owner: &Keypair,
    amount: u64,
    ...
    // Get the mint from the token account
    let mint = get_token_mint(client.clone(), token_account).await?;
    ...
    // Get token account data to determine decimals
    let account_data = client.get_account(token_account).await?;
    let token_account_state = TokenAccount::unpack(&account_data.data)?;
    let account = StateWithExtensionsOwned::<TokenAccount>::unpack(account_data.data)?;
    let confidential_transfer_state = account.get_extension::<ConfidentialTransferAccount>()?;
    
    // Call the confidential withdraw instruction
    let signature = token
        .confidential_transfer_withdraw_with_proof(
            token_account,                // Token account to withdraw from
